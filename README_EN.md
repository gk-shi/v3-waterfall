...
|       active       |                           boolean                            |           true           |    no    | *Confirm whether the current instance is activated when multiple `v3-waterfall` components are used similar to the `van-tabs` component.* |
|   swipeableDelay   |                            number                            |            0             |    no    | *Similar to the `swipeable` attribute of `van-tabs`, when enabled, it may fail to load successfully by immediately sliding to another tab upon rendering. This value needs to be set, recommended as 300, and adjusted according to the actual situation* |
